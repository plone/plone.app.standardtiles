...
def _render_cachekey(fun, self):
    ...
    if self.calendar.getUseSession():
        ...
        end = DateTime('%s/%s/1 23:59:59' % self.getNextMonth(year,
                                                              month)) - 1
...
class CalendarTile(Tile):
    ...
    def update(self):
        ...
        self.prevMonthYear, self.prevMonthMonth = self.getPreviousMonth(year,
                                                                        month)
        ...
        for week in weeks:
            for day in week:
                ...
                if day['event']:
                    ...
                    localized_date = [
                        self._ts.ulocalized_time(cur_date, context=context,
                                                 request=self.request)]
                    day['eventstring'] = '\n'.join(localized_date + [
                            ' %s' % self.getEventString(e) for e
                            in day['eventslist']])
